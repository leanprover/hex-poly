  ...
  have hfold : ∀ m acc,
    ...
    | succ m ih =>
        ...
        have hterm : term (scale a p) m = a * term p m := by
          ...
          by_cases hnm : n < m
          · simp only [hnm, ite_true]
            grind
          · simp only [hnm, ite_false, coeff_scale_semiring]
            grind
        rw [hterm]
        grind
  have h := hfold (n + 1) 0
...
theorem monomial_mul_monomial {S : Type _}
  ...
  have hterm : ∀ i,
    ...
    · have hcond : ¬ (i = m ∧ n = m + k) := by
        intro ⟨h1, h2⟩; omega
      rw [ite_eq_left hni, ite_eq_right hcond]
  ...
  · have hsimp : ∀ i,
    ...
    rw [ite_eq_left hm_lt, ite_eq_left hnmk]
  · have hzero_fold : ∀ (xs : List Nat) (acc : S),
      ...
      | cons i xs ih =>
          ...
          rw [ite_eq_right hcond, show acc + (0 : S) = acc by grind]
          exact ih _
    rw [hzero_fold, ite_eq_right hnmk]
    rfl
